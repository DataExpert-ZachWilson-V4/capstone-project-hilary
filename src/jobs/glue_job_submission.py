...
def display_logs_from(paginator, run_id, log_group, continuation_token):
...
def create_glue_job(
    ...
    **kwargs,
    ...
    script_path = upload_to_s3(script_path, s3_bucket, "jobscripts/hsh/" + script_path)
    if not script_path:
        raise ValueError("Uploading PySpark script to S3 failed")
    # Check to see if passed in any --conf parameters to override the output table
    output_table = (
    ...
    job_args = {
        ...
        "DefaultArguments": {"--conf": spark_string, "--extra-jars": extra_jars},
        "GlueVersion": "4.0",
        "WorkerType": "Standard",
        "NumberOfWorkers": 1,
    }
    ...
    if kafka_credentials is not None:
        ...
        arguments["--checkpoint_location"] = (
            f"""s3://{s3_bucket}/kafka-checkpoints/hsh/{job_name}"""
        )
